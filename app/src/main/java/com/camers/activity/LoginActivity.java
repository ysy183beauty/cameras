...
import com.camers.R;
import com.camers.util.Base64Utils;
import com.camers.util.Constant;
import com.camers.util.HttpCallbackListener;
import com.camers.util.HttpResult;
import com.camers.util.HttpUtil;
...
//       return password;   //解码一下
    ...
    public void login() {
        ...
        if (getAccount().isEmpty()){
            showCoverDialog("用户名不能为空");
            return;
        }
        if (getPassword().isEmpty()){
            showCoverDialog("密码不能为空");
            return;
        }
        //获取SharedPreferences对象，使用自定义类的方法来获取对象
        SharedPreferencesUtils helper = new SharedPreferencesUtils(this, "setting");
       ...
       Thread loginRunnable = new Thread() {
            ...
            public void run() {
                ...
                new HttpResult(Constant.BASEURL+"login",params,mHandler).getResult();
            }
        };
        loginRunnable.start();
    }
